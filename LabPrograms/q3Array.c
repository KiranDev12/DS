...
? i)Insert by position --> insert at the given position
? ii) Delete by position --> delete at given position
? iii)Insert by key --> search if key present or not... if present dont do anything, if not insert at last
? iv) Delete by key --> delete the given key element
? v) Insert by order --> insert within order
? vi) Search by key --> if found return found
? vii) Search by position --> accesing an element at particular index
? viii) Reverse the contents --> reverse
*/
...
int main()
{
    int choice, s = 0;
    int *p, *size;
    printf("Enter the size of the array,to which memory is allocated dynamically:\t");
    scanf("%d", &s);
    ...
    while (1)
    {
        ...
        case 2:
            ...
            case 2:
                ...
                break;
            case 3:
                searchInsertPtr(p, size);
                break;
            case 4:
                searchDeletionPtr(p, size);
                break;
            case 5:
                searchByKeyPtr(p, size);
                break;
            case 6:
                searchByPosPtr(p, size);
                break;
            case 7:
                reverseArrayPtr(p, size);
                break;
            case 8:
                sortInsertPtr(p, size);
                break;
            case 9:
                printf("\nCurrent Array is...\n");
                display(p, size);
